...
# Database MỚI: 08_seed_counters gọi các hàm (định nghĩa ở 03/16) mà thân hàm
# gọi app_require — hàm đó 17_auth mới tạo. Tắt kiểm tra thân hàm lúc tạo và
# đặt một app_require rỗng tạm thời; 17_auth thay bằng bản thật.
$prelude = @'
set check_function_bodies = off;
do $$ begin
  if to_regprocedure('app_require(text,text)') is null then
    execute 'create function app_require(p_module text, p_action text) returns void language plpgsql as $f$ begin end $f$';
  end if;
end $$;
'@

function Build([string]$name, [string[]]$files, [string[]]$header, [string]$pre = '') {
...
Build 'ALL_IN_ONE.sql' $order @(
  ...
  '  seed ở đây ghi đè master data đã sửa trong app.') $prelude
